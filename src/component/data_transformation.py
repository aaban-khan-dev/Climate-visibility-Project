...
class DataTransformation:
    ...
    def drop_schema_columns(self,dataframe:pd.DataFrame)->pd.DataFrame:
        ...
        Description : This Method reads the schema file and drops the columns accordingly
        Output : a pd.datafram with dropped columns
        """
        try:
            _schema_config = self.utils.read_schema_config_file()
            df = dataframe.drop(columns = _schema_config["drop_columns"],axis=1)
            return df
    ...
    def get_merged_batched_data(valid_data_dir:str)->pd.DataFrame:
        """
        Method Name :  get_merged_batched_data
        Description : This Method reads the batch files and merges them into a single dataframe
        Output : a pd.datafram with merged data
         """
    ...
    def initiate_data_transformation(self):
        ...
        Description : this method initiates the data tranformation component for the pipeline
        Output : data transformation aritfacts is created and returned
        """
        
        logging.info("Initiated data tranformation method for data tranformation class")
